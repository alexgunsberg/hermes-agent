...
def test_native_draft_streaming_default_is_legacy_edit_path():
    """Telegram draft streaming must be opt-in to avoid unsupported placeholders."""
...
def test_native_draft_streaming_requires_explicit_opt_in_and_private_chat():
    adapter = _make_adapter(extra={"native_drafts": True})
...
async def test_table_only_respects_rich_messages_opt_out():
    """Pipe tables must not auto-route to sendRichMessage when rich is off."""
    adapter = _make_adapter(extra={"rich_messages": False})
...
async def test_table_only_uses_legacy_with_default_config():
    """Default config must stay legacy-compatible: no rich table auto-upgrade."""
    config = PlatformConfig(enabled=True, token="fake-token")
    adapter = TelegramAdapter(config)
    bot = MagicMock()
    bot.do_api_request = AsyncMock(return_value=SimpleNamespace(message_id=123))
...
@pytest.mark.asyncio
async def test_dm_topic_resumed_send_table_stays_legacy_without_reply_anchor():
    """With rich off, resumed DM-topic table sends stay on legacy path."""
    adapter = _make_adapter(extra={"rich_messages": False})
...
@pytest.mark.asyncio
async def test_finalize_edit_table_stays_legacy_when_rich_off():
    adapter = _make_adapter(extra={"rich_messages": False})
