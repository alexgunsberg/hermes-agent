...
def test_run_slash_diagnostics_all_boards_ignores_worker_db_env(kanban_home, monkeypatch):
    ...
    kb.init_db()
    kb.create_board("profile-inbox")
    for slug, task_id, title in [
        (kb.DEFAULT_BOARD, "t_aaaabbbb", "default missing owner ledger"),
        ("profile-inbox", "t_ccccdddd", "profile inbox missing owner ledger"),
    ]:
    ...
    # must still inspect sibling boards/profile inboxes.
    ...
    assert by_id["t_ccccdddd"]["board"] == "profile-inbox"
