...
KANBAN_GUIDANCE = (
    ...
    "structured metadata (changed_files / tests_run / diff_path) plus a "
    "handoff (`from`, `to`, `accepted_by`, `next_action`, `stale_after`, "
    "`evidence`) into a `kanban_comment` first, then end with "
    "`kanban_block(reason=\"review-required: <one-line summary>\")`. The "
    "named project owner owns review closeout; do not leave the card as a "
    "nobody-owned human handoff.\n"
