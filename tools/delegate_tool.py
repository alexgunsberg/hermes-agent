...
def _load_config() -> dict:
    """Load delegation config without cold-importing cli.py.

    Delegate's dynamic schema builder runs during normal tool-schema discovery;
    importing ``cli`` on that path pulls in the full CLI startup stack and
    profile secret-source bootstrap even when the user is not invoking
    delegation. If the CLI module is already resident, use its in-memory config;
    otherwise use the lightweight persistent loader. Do NOT mutate the returned
    dict.
    """
